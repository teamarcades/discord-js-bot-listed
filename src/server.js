...
module.exports = async (client) => {
    ...
    const checkMaintence = async (req, res, next) => {
        ...
        if (d) {
            if (req.isAuthenticated()) {
                ...
                if (usercheck) {
                    if (usercheck.roles.cache.get(roles.owner)) {
                        next();
                    } else {
    ...
    app.get("/dc", (req, res) => {
        res.redirect('https://discord.gg/pBcS4fjStD');
    })

    app.get("/teamarcades", (req, res) => {
        res.redirect('https://discord.gg/team-arcades-935157109761388554');
    })

    app.get("/status", (req, res) => {
        res.redirect('https://status.watchbot.app/bot/1009896161961840745');
    })
        ...
        if (banned) {
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://botlist.teamarcades.xyz/user/${a.id}) logged into the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
            })
            req.session.destroy(() => {
                res.json({
                    login: false,
                    message: "You have been blocked from Team.",
                    logout: true
                })
                req.logout();
            });
            ...
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://botlist.teamarcades.xyz/user/${a.id}) Just Logged into the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    app.get("/partners", checkMaintence, (req, res) => {
        const Database = require("./packages/teamarcades.db/teamarcades-db.js");
    ...
    app.get("/robots.txt", function(req, res) {
        ...
        res.send(`Sitemap: https://botlist.teamarcades.xyz/sitemap.xml`);
    });

    app.get("/sitemap.xml", async function(req, res) {
        let link = "<url><loc>https://botlist.teamarcades.xyz/</loc></url>";
        let botdataforxml = await botsdata.find()
        botdataforxml.forEach(bot => {
            link += "\n<url><loc>https://botlist.teamarcades.xyz/bot/" + bot.botID + "</loc></url>";
        })
        ...
        if (!rBody['link']) {
            ...
            client.users.fetch(req.user.id).then(a => {
                client.channels.cache.get(channels.uptimelog).send(new Discord.MessageEmbed()
                    ...
                    .setDescription("Nuevo enlace agregado sistema de uptime.")
                    .setThumbnail(client.user.avatarURL)
                    .setColor("GREEN")
                    .addField("User;", `${a.tag} \`(${a.id})\``, true)
                    .addField("Codigo de Uptime:", updcode, true)
                    .addField("Limite de Uptime:", `${dd.length+1}/10`, true)
                )
                new uptimedata({
    ...
    app.post("/addbot", checkMaintence, checkAuth, async (req, res) => {
        ...
        client.users.fetch(rBody['botID']).then(a => {
            client.channels.cache.get(channels.botlog).send(`<@${req.user.id}> agregado **${a.tag}**`)
    ...
    const checkAdmin = async (req, res, next) => {
        if (req.isAuthenticated()) {
            if (client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(roles.owner) || client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(roles.moderator)) {
                next();
            } else {
    ...
    app.get("/admin/confirm/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(req.params.botID).then(bota => {
            client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> Se ha aprobado el bot llamado **${bota.tag}**. `)
            client.users.cache.get(botdata.ownerID).send(`Tu bot llamado **${bota.tag}** ha sido aprobado.`)
        });
    ...
    app.post("/admin/decline/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.ownerID).then(sahip => {
            client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>'s El bot llamado **${botdata.username}** ha sido rechazado. `)
            client.users.cache.get(botdata.ownerID).send(`Su bot llamado **${botdata.username}** ha sido rechazado.\nMotivo: **${rBody['reason']}**\nAutorizado: **${req.user.username}**`)
        })
        await botsdata.deleteOne({
            botID: req.params.botID,
            ownerID: botdata.ownerID
    ...
    app.get("/admin/certificate/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.botID).then(bota => {
            client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> su bot llamado **${bota.tag}** se le ha otorgado un certificado.`)
            client.users.cache.get(botdata.ownerID).send(`Su bot llamado **${bota.tag}** ha sido certificado.`)
        });
    ...
    app.post("/admin/certificate/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.botID).then(bota => {
            client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> su bot llamado **${bota.tag}** no se le ha otorgado un certificado.`)
            client.users.cache.get(botdata.ownerID).send(`Se rechazó la solicitud de certificado de su bot llamado **${bota.tag}**.\nRazón: **${rBody['reason']}**`)
        });
        await appsdata.deleteOne({
            botID: req.params.botID
        })
    ...
    app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`El usuario llamado **[${req.user.username}](https://botlist.teamarcades.xyz/user/${req.user.id})** agregó el código llamado **${rBody['codename']}** al sistema.`)
            .addField("Enlace de código", `https://botlist.teamarcades.xyz/code/${kod}`, true)
            .addField("Descripción de código", rBody['codedesc'], true)
            .addField("Categoria de código", rBody['category'], true)
        )
    ...
    app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            .setTitle("¡Código editado!").setColor("GREEN").setFooter(config.footer)
            .setDescription(`El usuario llamado **[${req.user.username}](https://botlist.teamarcades.xyz/user/${req.user.id})** edito el código llamado **${rBody['codename']}**.`)
            .addField("Enlace de código", `https://botlist.teamarcades.xyz/code/${kod}`, true)
            .addField("Descripcion de código", rBody['codedesc'], true)
            .addField("Categoria de código", rBody['category'], true)
        )
        res.redirect('/code/' + kod)
    });
    ...
    app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`🟠 El SISTEMA se ha cambiado a __mantenimiento__ debido a **${req.body.reason}**`).then(a => {
            new maintenceSchema({
                server: config.server.id,
    ...
    app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        const bakimsonaerdikardesTopic = new dc.MessageEmbed()
            .setAuthor("Team Arcades | Manager", client.user.avatarURL())
            .setThumbnail(client.user.avatarURL())
            .setColor("GREEN")
            .setDescription(`🟢 El equipo está **activo** de nuevo!\n[Haga clic para redirigir el sitio web](https://botlist.teamarcades.xyz/)`)
        ...
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
            a.edit(`~~ 🟠 El SISTEMA se ha cambiado a __mantenimiento__ debido a **${bakimdata.reason}** ~~`, bakimsonaerdikardesTopic)
        })
